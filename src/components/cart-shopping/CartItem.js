...
import image from '../../Mask Group (8).png'
export function CartItem({item}) {
    const dispatch = useDispatch()
    const iteam = {price:2719}
    ...
    return (
        <div className='py-4 px-2 sm:px-4 border-b'>
            <div className='flex justify-between'>
                <div className='flex'>
                    <img className='w-[60px]' src={image} alt={item.title}/>
            ...
            <div className='flex justify-between items-center relative mt-4 sm:mt-2 sm:justify-end'>
                <div className='relative bottom-0 right-0 text-lg sm:absolute sm:right-[300px]'>{iteam.price} P</div>
                ...
                <span className='text-lg'>{2719*item.quantity} P</span>
            </div>
        </div>
    )
}
